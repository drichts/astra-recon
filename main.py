...
rec_data = np.zeros((20, 24, 576, 576))
for i in range(20):
    astra.algorithm.run(alg_id, 10)

    # Get the result
    rec = astra.data3d.get(recon_id)
    print(f'CGLS: {(i+1)*10} Iterations')
    rec_data[i] = rec
...
np.save(r'/home/knoll/LDAData/recon_CGLS_v2.npy', rec_data)
...
for i in range(20):
    ...
    # Get the result
    rec = astra.data3d.get(recon_id)
    print(f'SIRT: {(i+1)*10} Iterations')
    rec_data[i] = rec
    # plt.figure(figsize=(8, 8))
    # plt.imshow(rec[15, :, :], vmin=0, vmax=0.006)
    # plt.show(block=True)
...
np.save(r'/home/knoll/LDAData/recon_SIRT_v2.npy', rec_data)
